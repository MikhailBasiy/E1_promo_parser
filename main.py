...
def prepare_site_data(site_prices: pd.DataFrame) -> pd.DataFrame:
    site_prices["Название_карточки"] = site_prices["Название_карточки"].str.replace(
        "Шкаф-купе ",
        "",
    )
    site_prices["Цвет_профиля"] = site_prices["Цвет_профиля"].str.replace(
        " профиль", ""
    )
    return site_prices
...
def main():
    ...
    site_prices.to_excel(
        f"collected_prices/prices_{fdate}.xlsx", engine="xlsxwriter", index=False
    )
